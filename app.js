...
function toggleCalibration(){
  ...
  if (!panel.classList.contains('hidden')) {
    ...
    // Reiniciar calibración automática si no está calibrado
...
function showSettings(){
  ...
  modal.innerHTML=`<div class="modal-box"><h3>Ajustes Avanzados</h3>
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  // Cargar todas las rutas existentes y seleccionarlas por defecto
  const allRoutes = getAllRoutes();
  state.selectedRouteIds = new Set(allRoutes.map(r => r.id));
  // Actualizar el botón de promediar según estado inicial
  updateAverageButton();
