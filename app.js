...
function addCalibrationSample(x,y,z) {
  ...
  state.gravityCalibrationSamples.push({x,y,z});
  if(Date.now()-state.calibrationStartTime>=2000||state.gravityCalibrationSamples.length>=120) {
    finalizeCalibration();
  }
}
    ...
    try {
      ...
      window.accelerometer.addEventListener('reading', ()=>{
        ...
        if(!state.orientationCalibrated) { addCalibrationSample(x,y,z); }
          ...
          processAccelerometerData(Math.abs(proj-9.8));
        }
      });
...
function openFullscreenPauseMenu() {
  document.getElementById('fsPauseMenu').classList.toggle('hidden');
}
function hideFullscreenPauseMenu() {
  document.getElementById('fsPauseMenu').classList.add('hidden');
}
function toggleCalibrationFromFullscreen() {
  hideFullscreenPauseMenu();
  toggleCalibration();
}
