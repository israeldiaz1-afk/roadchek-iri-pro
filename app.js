...
const VEHICLE_DATABASE = [
  ...
  { id: 'v34', name: 'Peugeot 307 SW (2002-2008)', category: 'Compacto', coefA: 2.15, coefB: 0.55, description: 'Familiar estable' },
  { id: 'v35', name: 'Volkswagen Passat (2012)', category: 'Sedán', coefA: 2.05, coefB: 0.50, description: 'Berlina equilibrada' },
  { id: 'v36', name: 'Citroën e-C4 (2021-2024)', category: 'SUV', coefA: 1.65, coefB: 0.40, description: 'SUV eléctrico confort' }
];

// ============ ESCALA INTELIGENTE DE COLORES ============
...
function initSensorChart() {
  ...
  state.sensorChart = new Chart(ctx, {
    ...
    data: {
      labels: [],
      datasets: [
        { label: 'Acel. Z (m/s²)', data: [], borderColor: '#00bcd4', yAxisID: 'y', tension: 0.3, pointRadius: 0 },
        { label: 'IRI Corregido', data: [], borderColor: '#e94560', yAxisID: 'y1', tension: 0.3, pointRadius: 0 }
      ]
    },
    options: {
      ...
      scales: {
        x: { display: false },
        y: { type: 'linear', display: true, position: 'left', title: { display: true, text: 'm/s²', color: '#00bcd4' }, min:0, max:20, ticks:{color:'#00bcd4'} },
        y1: { type: 'linear', display: true, position: 'right', title: { display: true, text: 'IRI', color: '#e94560' }, min:0, max:10, grid:{drawOnChartArea:false}, ticks:{color:'#e94560'} }
      },
...
function processAccelerometerData(verticalAccel) {
  ...
  if(state.sensorChart){
    ...
    state.sensorChart.data.datasets[0].data=state.chartDataZ; state.sensorChart.data.datasets[1].data=state.chartDataIRI;
...
function addCalibrationSample(x,y,z) {
  ...
  if(samples.length>0){ const last=samples[samples.length-1]; if(Math.abs(x-last.x)+Math.abs(y-last.y)+Math.abs(z-last.z)>0.5) return; }
  samples.push({x,y,z});
...
function startAccelerometer() {
  ...
  if('Accelerometer' in window) {
    try {
      ...
      window.accelerometer.start(); state.useDeviceOrientationFallback=false; return;
    }catch(e){}
  }
...
function enterFullscreenMode() {
  const main=document.getElementById('main-app'), fs=document.getElementById('fullscreen-measure');
  if(main) main.style.display='none';
  fs.classList.remove('hidden');
  if(!state.mapFullscreen) initFullscreenMap();
  setTimeout(()=>{
    state.mapFullscreen.invalidateSize();
    state.fullscreenRouteLines.forEach(l=>state.mapFullscreen.removeLayer(l)); state.fullscreenRouteLines=[];
    state.currentSegmentPoints=[]; state.currentSegmentDistance=0; state.currentSegmentIRISum=0; state.currentSegmentPointCount=0;
...
function exitFullscreenMode() {
  ...
  const main=document.getElementById('main-app'); if(main) main.style.display='';
}
...
function startMeasurement() {
  ...
  state.lastPosition=null; state.chartDataZ=[]; state.chartDataIRI=[]; state.dynamicBuffer=[]; state.dynamicThresholds=null;
  ...
  if(state.sensorChart){state.sensorChart.data.labels=[]; state.sensorChart.data.datasets[0].data=[]; state.sensorChart.data.datasets[1].data=[]; state.sensorChart.update();}
  startGPS(); startAccelerometer(); updateTimer();
  setTimeout(()=>enterFullscreenMode(),50);
}
function pauseMeasurement() {
  state.isPaused=true; document.getElementById('pauseStopControls').classList.add('hidden');
  document.getElementById('btnResume').classList.remove('hidden');
  if(window.accelerometer) window.accelerometer.stop(); exitFullscreenMode();
}
function resumeMeasurement() {
  state.isPaused=false; document.getElementById('btnResume').classList.add('hidden');
  document.getElementById('pauseStopControls').classList.remove('hidden');
  if(window.accelerometer) window.accelerometer.start(); enterFullscreenMode();
}
function stopMeasurement() {
  state.isMeasuring=false; state.isPaused=false; stopSensors();
  ...
  document.getElementById('btnResume').classList.add('hidden');
  document.getElementById('btnStart').classList.remove('hidden'); exitFullscreenMode();
...
// ============ GPS ============
function startGPS() {
  if('geolocation' in navigator) state.watchId=navigator.geolocation.watchPosition(updateGPSPosition, err=>showToast('Error GPS: '+err.message), {enableHighAccuracy:true, maximumAge:1000, timeout:10000});
}
...
function toggleCalibration(){document.getElementById('calibrationPanel').classList.toggle('hidden');document.getElementById('coefA').value=config.coefA;document.getElementById('coefB').value=config.coefB;document.getElementById('coefAVal').textContent=config.coefA.toFixed(2);document.getElementById('coefBVal').textContent=config.coefB.toFixed(2);}
function applyCalibration(){config.coefA=parseFloat(document.getElementById('coefA').value);config.coefB=parseFloat(document.getElementById('coefB').value);saveConfig();document.getElementById('calibrationPanel').classList.add('hidden');showToast('Calibración guardada');if(state.activeVehicleId){const cur=getAllVehicles().find(v=>v.id===state.activeVehicleId);if(cur&&(Math.abs(cur.coefA-config.coefA)>0.05||Math.abs(cur.coefB-config.coefB)>0.05)){state.activeVehicleId=null;localStorage.removeItem('roadcheck_active_vehicle');document.getElementById('currentVehicleName').textContent='Personalizado';}}}
function showSettings(){const modal=document.createElement('div');modal.className='modal';modal.innerHTML=`<div class="modal-content"><h3>Ajustes Avanzados</h3><label>Coef. corrección velocidad: <span id="speedKVal">${config.speedCorrectionK}</span></label><input type="range" id="speedK" min="0" max="0.05" step="0.001" value="${config.speedCorrectionK}" oninput="document.getElementById('speedKVal').textContent=parseFloat(this.value).toFixed(3)"><label>Velocidad referencia (km/h): <span id="refSpeedVal">${config.referenceSpeed}</span></label><input type="range" id="refSpeed" min="30" max="120" step="5" value="${config.referenceSpeed}" oninput="document.getElementById('refSpeedVal').textContent=this.value"><button class="btn btn-start" style="width:100%;margin-top:15px;" id="saveSettingsBtn">Guardar</button><button class="btn btn-secondary" style="width:100%;margin-top:5px;" onclick="this.closest('.modal').remove()">Cancelar</button></div>`;document.body.appendChild(modal);document.getElementById('saveSettingsBtn').addEventListener('click',()=>{config.speedCorrectionK=parseFloat(document.getElementById('speedK').value);config.referenceSpeed=parseInt(document.getElementById('refSpeed').value);saveConfig();modal.remove();showToast('Ajustes guardados');});}
function updateVehicleDisplay(v){document.getElementById('currentVehicleName').textContent=v?v.name:'Personalizado';document.getElementById('coefA').value=config.coefA;document.getElementById('coefB').value=config.coefB;document.getElementById('coefAVal').textContent=config.coefA.toFixed(2);document.getElementById('coefBVal').textContent=config.coefB.toFixed(2);}

// ============ GARAJE ============
function loadGarage(){const all=getAllVehicles();const cats=['Compacto','Sedán','SUV','Deportivo','Pick-up','Personalizado'];let html='';cats.forEach(cat=>{const vehs=all.filter(v=>v.category===cat);if(vehs.length){html+=`<h4 style="margin:10px 0 5px;color:#e94560;">${cat}</h4>`;vehs.forEach(v=>{const act=state.activeVehicleId===v.id;html+=`<div class="garage-item ${act?'active':''}" onclick="selectVehicle('${v.id}')"><div><strong>${v.name}</strong><br><small>a: ${v.coefA.toFixed(2)} | b: ${v.coefB.toFixed(2)}</small></div><div>${v.id.startsWith('vc')?`<button class="btn-small" onclick="event.stopPropagation();deleteCustomVehicle('${v.id}')">🗑️</button>`:''}</div></div>`;})}});document.getElementById('garageList').innerHTML=html;}
function selectVehicle(id){const v=getAllVehicles().find(v=>v.id===id);if(!v)return;config.coefA=v.coefA;config.coefB=v.coefB;state.activeVehicleId=id;localStorage.setItem('roadcheck_active_vehicle',id);saveConfig();updateVehicleDisplay(v);loadGarage();showToast(`Seleccionado: ${v.name}`);}
function showAddVehicleModal(){document.getElementById('addVehicleModal').classList.remove('hidden');document.getElementById('vehicleName').value='';document.getElementById('vehicleCoefA').value=config.coefA;document.getElementById('vehicleCoefB').value=config.coefB;document.getElementById('vehicleCoefAVal').textContent=config.coefA.toFixed(2);document.getElementById('vehicleCoefBVal').textContent=config.coefB.toFixed(2);document.getElementById('btnSaveVehicle').onclick=saveNewVehicle;}
function closeVehicleModal(){document.getElementById('addVehicleModal').classList.add('hidden');}
function saveNewVehicle(){const name=document.getElementById('vehicleName').value.trim();if(!name){showToast('Introduce un nombre');return;}const a=parseFloat(document.getElementById('vehicleCoefA').value),b=parseFloat(document.getElementById('vehicleCoefB').value);const cust=getCustomVehicles();cust.push({id:'vc'+Date.now(),name,category:'Personalizado',coefA:a,coefB:b,description:'Personalizado'});saveCustomVehicles(cust);closeVehicleModal();selectVehicle(cust[cust.length-1].id);loadGarage();}
function deleteCustomVehicle(id){if(!id.startsWith('vc'))return;let cust=getCustomVehicles().filter(v=>v.id!==id);saveCustomVehicles(cust);if(state.activeVehicleId===id){state.activeVehicleId=null;localStorage.removeItem('roadcheck_active_vehicle');document.getElementById('currentVehicleName').textContent='Personalizado';}loadGarage();showToast('Vehículo eliminado');}

// ============ HISTORIAL ============
function loadHistory(){const routes=getAllRoutes();const cont=document.getElementById('historyList');if(!routes.length){cont.innerHTML='<p style="text-align:center;">No hay rutas guardadas</p>';return;}cont.innerHTML=routes.map(r=>`<div class="history-item" onclick="viewRouteDetail('${r.id}')"><div><strong>${formatDate(r.date)}</strong><br><small>${r.totalDistance.toFixed(0)} m | IRI corr: ${r.avgIRICorrected?.toFixed(2)||'N/A'}</small></div><div class="history-actions"><button class="btn-small" onclick="event.stopPropagation();exportRouteCSV('${r.id}')">📥</button><button class="btn-small" onclick="event.stopPropagation();deleteRoute('${r.id}')">🗑️</button></div></div>`).join('');}
let currentRouteId=null;
function viewRouteDetail(id){const r=getAllRoutes().find(r=>r.id===id);if(!r)return;currentRouteId=id;document.getElementById('routeModalTitle').textContent=`Ruta del ${formatDate(r.date)}`;document.getElementById('routeDate').textContent=formatDate(r.date);document.getElementById('routeDistance').textContent=r.totalDistance.toFixed(1)+' m';document.getElementById('routeIRIMeasuredAvg').textContent=r.avgIRIMeasured?.toFixed(2)||'N/A';document.getElementById('routeIRICorrectedAvg').textContent=r.avgIRICorrected?.toFixed(2)||'N/A';const segList=document.getElementById('routeSegmentList');if(r.segments?.length){segList.innerHTML='<p><strong>Segmentos:</strong></p>'+r.segments.map((seg,i)=>`<div style="background:#0f3460;padding:5px;margin:3px 0;border-radius:4px;display:flex;align-items:center;"><span style="background:${seg.color};width:12px;height:12px;display:inline-block;margin-right:6px;"></span>Seg ${i+1}: ${seg.distance.toFixed(0)}m | IRI corr: ${seg.iriCorrectedAvg.toFixed(2)}</div>`).join('');}else segList.innerHTML='<p>No hay segmentos</p>';const footer=document.querySelector('#routeModal .controls');if(footer){footer.innerHTML=`<button class="btn btn-export" onclick="exportRouteCSV('${r.id}')">📥 CSV</button><button class="btn btn-start" onclick="exportRouteGeoJSON('${r.id}')">🗺️ GeoJSON</button><button class="btn btn-stop" onclick="deleteRoute('${r.id}')">🗑️ Borrar</button><button class="btn btn-secondary" onclick="showRouteOnGlobalMap('${r.id}')">📍 Ver en mapa</button><button class="btn btn-secondary" onclick="closeModal()">Cerrar</button>`;}document.getElementById('routeModal').classList.remove('hidden');}
function closeModal(){document.getElementById('routeModal').classList.add('hidden');currentRouteId=null;}
function deleteRoute(id){if(confirm('¿Eliminar ruta?')){deleteRouteById(id);if(currentRouteId===id)closeModal();loadHistory();showToast('Ruta eliminada');}}
function clearAllHistory(){if(confirm('¿Borrar todo?')){clearAllRoutes();loadHistory();showToast('Historial borrado');}}
function exportRouteCSV(id){const r=getAllRoutes().find(r=>r.id===id);if(!r?.points)return;let csv='Timestamp,Lat,Long,Speed,IRI_Measured,IRI_Corrected\n';r.points.forEach(p=>csv+=`${p.timestamp},${p.lat},${p.lon},${p.speed},${p.iri_measured},${p.iri_corrected}\n`);const blob=new Blob([csv],{type:'text/csv'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`ruta_${id}.csv`;a.click();}
function exportRouteGeoJSON(id){const r=getAllRoutes().find(r=>r.id===id);if(!r?.points){showToast('No hay datos');return;}const feats=[];if(r.points.length>1)feats.push({type:'Feature',properties:{id:r.id,fecha:r.date,distancia_total_m:r.totalDistance,iri_medido_prom:r.avgIRIMeasured,iri_corregido_prom:r.avgIRICorrected,tipo:'ruta_completa'},geometry:{type:'LineString',coordinates:r.points.map(p=>[p.lon,p.lat])}});if(r.segments)r.segments.forEach((seg,i)=>{if(seg.points?.length>=2)feats.push({type:'Feature',properties:{id:r.id,segmento:i+1,distancia_m:seg.distance,iri_medido_prom:seg.iriMeasuredAvg,iri_corregido_prom:seg.iriCorrectedAvg,velocidad_media:seg.speedAvg,color_hex:seg.color,tipo:'segmento'},geometry:{type:'LineString',coordinates:seg.points.map(p=>[p.lon,p.lat])}})});r.points.forEach(p=>feats.push({type:'Feature',properties:{id:r.id,timestamp:new Date(p.timestamp).toISOString(),lat:p.lat,lon:p.lon,speed_kmh:p.speed,iri_measured:p.iri_measured,iri_corrected:p.iri_corrected,tipo:'punto'},geometry:{type:'Point',coordinates:[p.lon,p.lat]}}));const geo={type:'FeatureCollection',features:feats},blob=new Blob([JSON.stringify(geo,null,2)],{type:'application/geo+json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`ruta_${id}.geojson`;a.click();showToast('GeoJSON exportado');}
...
function loadGlobalMapTab(){const routes=getAllRoutes();const container=document.getElementById('routeCheckboxes');if(!container)return;let html='';routes.forEach(r=>{const checked=state.selectedRouteIds.has(r.id)?'checked':'';html+=`<label class="checkbox-label"><input type="checkbox" value="${r.id}" ${checked} onchange="handleRouteCheckbox(this)"> ${formatDate(r.date)} (${r.totalDistance.toFixed(0)} m)</label>`;});container.innerHTML=html;}
function handleRouteCheckbox(cb){if(cb.checked)state.selectedRouteIds.add(cb.value);else state.selectedRouteIds.delete(cb.value);refreshGlobalMap();}
function toggleAllRoutes(selectAll){const cbs=document.querySelectorAll('#routeCheckboxes input[type=checkbox]');cbs.forEach(cb=>{cb.checked=selectAll;if(selectAll)state.selectedRouteIds.add(cb.value);else state.selectedRouteIds.delete(cb.value);});refreshGlobalMap();}
function computeAverageOverlaps(){state.showAverage=true;refreshGlobalMap();}
...
function updateGlobalMap(){
  ...
  if(state.showAverage){
    ...
    selectedRoutes.forEach(route=>{
      if(route.segments?.length) route.segments.forEach(seg=>{const iri=mode==='iri_measured'?seg.iriMeasuredAvg:seg.iriCorrectedAvg;L.polyline(seg.points.map(p=>[p.lat,p.lng]),{color:getIRIColor(iri),weight:5,opacity:0.8}).addTo(state.mapGlobal).on('click',()=>showSegmentInfo({...seg,iri,routeId:route.id}));});
...
function computeOverlappedSegments(routes,mode){
  ...
  for(let i=0;i<allSegs.length;i++){
    ...
    const base=allSegs[i],overlapping=[base];let sumIRI=base.iri,count=1;
    for(let j=i+1;j<allSegs.length;j++){
      if(processed.has(j)||base.routeId===allSegs[j].routeId)continue;
      if(haveOverlappingPoints(base.points,allSegs[j].points,10)){overlapping.push(allSegs[j]);sumIRI+=allSegs[j].iri;count++;processed.add(j);}
    }
    if(count>1) result.push({points:base.points,iri:sumIRI/count,iriMeasuredAvg:null,iriCorrectedAvg:sumIRI/count,speedAvg:null,distance:base.distance,color:getIRIColor(sumIRI/count),routeId:'average',tipo:'promedio'});
    else result.push(base);
    processed.add(i);
  }
  return result;
}
function haveOverlappingPoints(pts1,pts2,thr){for(const p1 of pts1)for(const p2 of pts2)if(calculateDistance(p1.lat,p1.lng,p2.lat,p2.lng)<=thr)return true;return false;}
...
document.addEventListener('DOMContentLoaded',()=>{
  initMeasureMap(); initGlobalMap(); initSensorChart();
