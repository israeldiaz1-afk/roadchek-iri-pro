...
let state = {
  ...
  mapMeasure: null, mapGlobal: null, measureRouteLine: null, currentMarker: null,
...
function toggleMapExpand() {
  ...
  if (!state.mapExpanded) {
    ...
    mapContainer.style.width = ''; mapContainer.style.height = '160px'; mapContainer.style.zIndex = '';
...
function startMeasurement() {
  if (!state.activeVehicleId) {
    showToast('Selecciona un vehículo primero');
    switchTab('garage');
    return;
  }
...
function updateHeaderVehicle() {
  const el = document.getElementById('headerVehicle');
  if (!el) return;
  if (state.activeVehicleId) {
    const v = getAllVehicles().find(v=>v.id===state.activeVehicleId);
    el.textContent = v ? '🚗 ' + v.name : '🚗 Sin vehículo';
  } else {
    el.textContent = '🚗 Sin vehículo';
  }
}
function showSettings(){
  const modal=document.createElement('div'); modal.className='modal-overlay';
  modal.innerHTML=`<div class="modal-box"><h3>Ajustes</h3>
    <h4>Corrección velocidad</h4>
    <label>k: <span id="speedKVal">${config.speedCorrectionK}</span></label>
    ...
    <label>Vel. ref: <span id="refSpeedVal">${config.referenceSpeed}</span></label>
    ...
    <h4>Vehículo</h4>
    <label>a: <span id="coefAVal2">${config.coefA.toFixed(2)}</span></label>
    ...
    <label>b: <span id="coefBVal2">${config.coefB.toFixed(2)}</span></label>
    ...
    <button class="btn btn-primary" style="width:100%;margin-top:12px;" id="saveSettingsBtn">Guardar</button>
    <button class="btn btn-secondary" style="width:100%;margin-top:4px;" onclick="this.closest('.modal-overlay').remove()">Cancelar</button></div>`;
...
function selectVehicle(id){
  ...
  saveConfig(); loadGarage(); updateHeaderVehicle();
...
function deleteCustomVehicle(id){
  ...
  loadGarage(); updateHeaderVehicle(); showToast('Vehículo eliminado');
}
...
function loadHistory(){
  ...
  if(!routes.length){cont.innerHTML='<p class="text-muted" style="text-align:center;">No hay rutas</p>';return;}
...
function updateGlobalMap(){
  ...
  if(selectedRoutes.length===0) selectedRoutes = [routes[routes.length-1]]; // última ruta por defecto
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  // Cargar rutas y seleccionar la última por defecto
  ...
  updateHeaderVehicle();
  document.getElementById('segmentLength').addEventListener('input', updateSegmentLabel);
