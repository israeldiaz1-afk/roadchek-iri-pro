...
function startAccelerometer() {
  // Intento 1: API moderna con includeGravity: false
  if ('Accelerometer' in window) {
    try {
      ...
      window.accelerometer.addEventListener('error', (e) => {
        console.warn('Error en Accelerometer, usando fallback:', e.error);
        fallbackToDeviceMotion();
      });
      window.accelerometer.start();
      return;
    } catch (e) {
      console.warn('No se pudo crear Accelerometer, usando fallback:', e);
    }
  }
  // Intento 2: Fallback con DeviceMotionEvent (sin gravedad)
  fallbackToDeviceMotion();
}
...
function fallbackToDeviceMotion() {
  window.addEventListener('deviceorientation', event => {
    ...
    // Usamos directamente 'acceleration', que NO incluye la gravedad
    const dynamicZ = event.acceleration?.z || 0;
