...
const DEFAULT_CONFIG = {
  ...
  noiseFloor: 0.12  // umbral mínimo de aceleración para ignorar ruido
};
...
function processAccelerometerData(verticalAccel) {
  // Aplicar umbral de ruido mínimo
...
function enterFullscreenMode() {
  const el = document.getElementById('fullscreen-measure');
  if (el.requestFullscreen) {
    el.requestFullscreen().catch(() => {
      // fallback si requestFullscreen falla
...
// Asegurar que al salir de pantalla completa con el botón atrás no se rompa
document.addEventListener('fullscreenchange', () => {
  if (!document.fullscreenElement) {
    // Si estamos midiendo, no ocultamos, solo se sale de fullscreen nativo
  }
});
...
function showSettings(){const modal=document.createElement('div');modal.className='modal-overlay';modal.innerHTML=`<div class="modal-box"><h3>Ajustes Avanzados</h3><label>Coef. corrección velocidad: <span id="speedKVal">${config.speedCorrectionK}</span></label><input type="range" id="speedK" min="0" max="0.05" step="0.001" value="${config.speedCorrectionK}" oninput="document.getElementById('speedKVal').textContent=parseFloat(this.value).toFixed(3)"><label>Velocidad referencia (km/h): <span id="refSpeedVal">${config.referenceSpeed}</span></label><input type="range" id="refSpeed" min="30" max="120" step="5" value="${config.referenceSpeed}" oninput="document.getElementById('refSpeedVal').textContent=this.value"><button class="btn btn-primary" style="width:100%;margin-top:15px;" id="saveSettingsBtn">Guardar</button><button class="btn btn-secondary" style="width:100%;margin-top:5px;" onclick="this.closest('.modal-overlay').remove()">Cancelar</button></div>`;document.body.appendChild(modal);document.getElementById('saveSettingsBtn').addEventListener('click',()=>{config.speedCorrectionK=parseFloat(document.getElementById('speedK').value);config.referenceSpeed=parseInt(document.getElementById('refSpeed').value);saveConfig();modal.remove();showToast('Ajustes guardados');});}
function updateVehicleDisplay(v){document.getElementById('currentVehicleName').textContent=v?v.name:'Personalizado';document.getElementById('coefA').value=config.coefA;document.getElementById('coefB').value=config.coefB;document.getElementById('coefAVal').textContent=config.coefA.toFixed(2);document.getElementById('coefBVal').textContent=config.coefB.toFixed(2);}

// ============ GARAJE ============
function loadGarage(){const all=getAllVehicles();const cats=['Compacto','Sedán','SUV','Deportivo','Pick-up','Personalizado'];let html='';cats.forEach(cat=>{const vehs=all.filter(v=>v.category===cat);if(vehs.length){html+=`<h4 style="margin:10px 0 5px;color:#f59e0b;">${cat}</h4>`;vehs.forEach(v=>{const act=state.activeVehicleId===v.id;html+=`<div class="list-item ${act?'active':''}" onclick="selectVehicle('${v.id}')"><div><strong>${v.name}</strong><br><small>a: ${v.coefA.toFixed(2)} | b: ${v.coefB.toFixed(2)}</small></div><div>${v.id.startsWith('vc')?`<button class="btn btn-sm btn-danger" onclick="event.stopPropagation();deleteCustomVehicle('${v.id}')">🗑️</button>`:''}</div></div>`;})}});document.getElementById('garageList').innerHTML=html;}
...
// ============ HISTORIAL ============
function loadHistory(){const routes=getAllRoutes();const cont=document.getElementById('historyList');if(!routes.length){cont.innerHTML='<p class="text-muted" style="text-align:center;">No hay rutas guardadas</p>';return;}cont.innerHTML=routes.map(r=>`<div class="list-item" onclick="viewRouteDetail('${r.id}')"><div><strong>${formatDate(r.date)}</strong><br><small>${r.totalDistance.toFixed(0)} m | IRI corr: ${r.avgIRICorrected?.toFixed(2)||'N/A'}</small></div><div class="history-actions"><button class="btn btn-sm btn-secondary" onclick="event.stopPropagation();exportRouteCSV('${r.id}')">📥</button><button class="btn btn-sm btn-danger" onclick="event.stopPropagation();deleteRoute('${r.id}')">🗑️</button></div></div>`).join('');}
let currentRouteId=null;
function viewRouteDetail(id){const r=getAllRoutes().find(r=>r.id===id);if(!r)return;currentRouteId=id;document.getElementById('routeModalTitle').textContent=`Ruta del ${formatDate(r.date)}`;document.getElementById('routeDate').textContent=formatDate(r.date);document.getElementById('routeDistance').textContent=r.totalDistance.toFixed(1)+' m';document.getElementById('routeIRIMeasuredAvg').textContent=r.avgIRIMeasured?.toFixed(2)||'N/A';document.getElementById('routeIRICorrectedAvg').textContent=r.avgIRICorrected?.toFixed(2)||'N/A';const segList=document.getElementById('routeSegmentList');if(r.segments?.length){segList.innerHTML='<p><strong>Segmentos:</strong></p>'+r.segments.map((seg,i)=>`<div style="background:rgba(0,0,0,0.2);padding:5px;margin:3px 0;border-radius:8px;display:flex;align-items:center;"><span style="background:${seg.color};width:12px;height:12px;display:inline-block;margin-right:6px;border-radius:2px;"></span>Seg ${i+1}: ${seg.distance.toFixed(0)}m | IRI corr: ${seg.iriCorrectedAvg.toFixed(2)}</div>`).join('');}else segList.innerHTML='<p>No hay segmentos</p>';const footer=document.querySelector('#routeModal .controls');if(footer){footer.innerHTML=`<button class="btn btn-sm btn-primary" onclick="exportRouteCSV('${r.id}')">📥 CSV</button><button class="btn btn-sm btn-accent" onclick="exportRouteGeoJSON('${r.id}')">🗺️ GeoJSON</button><button class="btn btn-sm btn-danger" onclick="deleteRoute('${r.id}')">🗑️ Borrar</button><button class="btn btn-sm btn-secondary" onclick="showRouteOnGlobalMap('${r.id}')">📍 Ver en mapa</button><button class="btn btn-sm btn-secondary" onclick="closeModal()">Cerrar</button>`;}document.getElementById('routeModal').classList.remove('hidden');}
...
function loadGlobalMapTab(){const routes=getAllRoutes();const container=document.getElementById('routeCheckboxes');if(!container)return;let html='';routes.forEach(r=>{const checked=state.selectedRouteIds.has(r.id)?'checked':'';html+=`<label class="custom-checkbox"><input type="checkbox" value="${r.id}" ${checked} onchange="handleRouteCheckbox(this)"> ${formatDate(r.date)} (${r.totalDistance.toFixed(0)} m)</label>`;});container.innerHTML=html;}
