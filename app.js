...
function calibratePhonePosition() {
  // Forzar una nueva calibración de orientación
  if (state.sensorActive) {
    // Reiniciar el proceso de calibración
    startOrientationCalibration();
  } else {
    showToast('El sensor no está activo. Reinicia la app.');
  }
}
...
function showSettings(){
  ...
  document.getElementById('saveSettingsBtn').addEventListener('click',()=>{
    ...
    saveConfig();
    // Si el vehículo activo no coincide, desvincularlo
...
function updateVehicleDisplay(v){
  ...
  // También actualizar los sliders del modal de ajustes si está abierto
  const a2=document.getElementById('coefA2');
  const b2=document.getElementById('coefB2');
